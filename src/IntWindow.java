...
public class IntWindow {
    ...
    /**
     * Used for when window is not filled but reached end of file - assumingly 0
     * - [last line's start]
     */
    ...
    private double calcMedian() {
        ...
        if (windowSize % 2 == 0) { //if even, average middle two entries
            ...
            //System.out.println("Median is " +(sortedRA.get(middle).getReadCount() + sortedRA.get(middle + 1).getReadCount()) / 2 );
            return (sortedRA.get(middle).getReadCount() + sortedRA.get(middle + 1).getReadCount()) / 2;
        } else {
            middle = windowSize / 2;
            //System.out.println("Median is " + sortedRA.get(middle).getReadCount());
    ...
    /**
     * Calculate the cMBF (1 - exp(-Z^2/2), Z = RC/M ) for the position using
    ...
    public void incrCenter() {
        ...
        if (!endOfChrom && winRA.get(0).getStart() < start) { //remove first element in RA if moving out of range and haven't reached end of file
            mf.removeNum(winRA.get(0).getReadCount());
            winRA.remove(0);
        }
    }

    public double calccMBF2() throws Exception { //currently uses mean
        double noiseEst = medianMult * mf.findMedian();
        if (noiseEst == 0) {
            throw new Exception("Noise level estimated to be 0: \n"
                    + chromNum + "\t" + start + "\t" + end + "\t" + pos);
        }
